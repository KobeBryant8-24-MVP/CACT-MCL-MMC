    ...
    for epoch in range(args.num_epoch + 1):
        ...
        with tqdm(train_loader) as td:
            for batch_image, text_input_ids, text_token_type_ids, text_attention_mask, batch_label in td:
                ...
                #loss, logit_m = model(text, image, None, labels)
                loss_mul, loss_t, loss_v, kl_loss, logit_m = model(text, image, None, labels)
                
                loss = loss_mul + loss_v + loss_t + kl_loss
                # print(loss)
